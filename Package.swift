...
let package = Package(
  ...
  products: [
    ...
    .library(name: "VisionClient", targets: ["VisionClient"]),

    .executable(name: "Usage", targets: ["Usage"]),
  ],
  dependencies: [
    .package(url: "https://github.com/googleapis/google-auth-library-swift.git", from: "0.5.2"),
  ...
  targets: [
    ...
    .target(name: "VisionClient", dependencies: ["GoogleAPIRuntime", "Commander"], path: "Sources/Client"),
    .target(name: "Usage", dependencies: ["VisionClient"], path: "Sources/Usage")
  ]
)
